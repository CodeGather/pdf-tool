...
func renderWholePagePDF(inputFile, outputDir, format string, dpi float64, timing, progressEnabled bool, quality int) error {
	...
	// 先用 pdfcpu 获取页数（不需 open/close fitz.Document）。
	// pdftoppm 的输出文件数就是页数。
	conf := model.NewDefaultConfiguration()
	conf.Cmd = model.EXTRACTIMAGES
	file, err := os.Open(inputFile)
	...
	// pdftoppm 渲染全部页面，比 go-fitz (MuPDF CGo) 快 2x 左右。
	// 它是独立 C 进程，无 CGo 信号栈问题，串行调用完全安全。
	pdftoppmFormat := ext
	...
	if runErr := cmd.Run(); runErr != nil {
		log.Printf("pdftoppm failed: %v, falling back to go-fitz", runErr)
	...
	// pdftoppm 输出为 outputDir/page-01.ext, page-02.ext, ...（自动按页数决定前导零位数）。
	// 遍历重命名为 page_001_image_001.ext 格式。
