...
func main() {
	...
	if *replaceEnabled {
		if *replaceDir != "" {
			if err := cmd.RunReplaceDir(*replaceDir, *replaceOutput, *parallelPct, *metaJSONEnabled); err != nil {
				fmt.Fprintf(os.Stderr, "批量合成失败：%v\n", err)
				os.Exit(1)
			}
		} else {
			if err := cmd.Run(*replaceJSON, *replaceOutput, *replaceFont, *replaceBaseDir, *parallelPct, *metaJSONEnabled); err != nil {
