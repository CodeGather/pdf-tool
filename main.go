...
func main() {
	...
	replacePrev := flag.Int("prev", 2, "替换模式：上一档信息显示，2=显示，1=不显示")
	flag.Usage = func() {
	...
	if *replaceEnabled {
		if *replaceDir != "" {
			if err := cmd.RunReplaceDir(*replaceDir, *replaceOutput, *parallelPct, *metaJSONEnabled, *replacePrev); err != nil {
				fmt.Fprintf(os.Stderr, "批量合成失败：%v\n", err)
				os.Exit(1)
			}
		} else {
			if err := cmd.Run(*replaceJSON, *replaceOutput, *replaceFont, *replaceBaseDir, *parallelPct, *metaJSONEnabled, *replacePrev); err != nil {
